...
class SearchIndex:
    ...
    def cal_weight(self, term, freq):
        return self.cal_tfs(freq) * self.cal_idf(term)

    def normalize_vector(self, terms):
        factor = 0
        for key, value in terms.iteritems():
            weight = self.cal_weight(key, value)
            terms[key] = weight
            factor += weight ** 2
        return {key: (value/factor) for key, value in terms.iteritems()} if factor else {}
    ...
    def search(self, query):
        ...
        query_vector = self.normalize_vector(terms)
